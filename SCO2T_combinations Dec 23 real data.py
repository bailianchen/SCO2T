...
if Only_integer=='Y':
    for i in range (Depth.shape[0]):
        numberOfWells[i] = int(decimalWells[i] + 1.00000000001)
else:
...
with open("Results_combination_full"  + ".csv", "w") as out_file:
    writer = csv.DictWriter(out_file, fieldnames = ["ID", "Name", "x","y","Depth","Pressure","Thickness","Permeability","Porosity","Geothermal_gradient","Temperature","Area","Max_pressure","Max_rate","Injection period","CO2 density","CO2 viscosity","Injection_rate","Plume radius","Plume volume","Extracted water","ROM","Reservoir capability","Well injection rate","Plume radius","Plume volume","Extracted water","Number of Wells","Dynamic reservoir capability","Static reservoir capability","Dynamic efficiency factor","Total cost", 'SiteFixedCost','SiteVarOM','WellFixedCost','WellFixedOM','WellVarOM'])
    writer.writeheader()
    for i in range(Depth.shape[0]):
        out_string = ""
        out_string += str(i+1)
...
for i in range (totalCost.shape[0]):
    if totalCost[i]>100:
        totalCost[i]=100
...
axe.hist(totalCost,bins=100)
axe.set_xlabel('Total cost ($/tCO2)',color='k', fontsize=12);
